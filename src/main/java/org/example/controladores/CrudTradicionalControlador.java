...
public class CrudTradicionalControlador extends BaseControlador {
    ...
    private EstudianteServices estudiante = EstudianteServices.getInstancia();
        ...
        app.routes(() -> {
            path("/crud-simple/", () -> {
                ...
                get("/listar", ctx -> {
                    ...
                    List<Estudiante> lista = estudiante.consultaNativa();
                    //
                ...
                post("/crear", ctx -> {
                    ...
                    estudiante.crear(tmp); //puedo validar, existe un error enviar a otro vista.
                    ctx.redirect("/crud-simple/");
                });

                get("/visualizar/{matricula}", ctx -> {
                    Estudiante estudiante = this.estudiante.find(ctx.pathParamAsClass("matricula", Integer.class).get());
                    //
                    Map<String, Object> modelo = new HashMap<>();
                ...
                post("/editar", ctx -> {
                    ...
                    Estudiante tmp = new Estudiante(matricula, nombre, carrera);
                    //realizar algún tipo de validación...
                    estudiante.editar(tmp); //puedo validar, existe un error enviar a otro vista.
                    ctx.redirect("/crud-simple/");
                });
                ...
                get("/eliminar/{matricula}", ctx -> {
                    estudiante.eliminar(ctx.pathParamAsClass("matricula", Integer.class).get());
                    ctx.redirect("/crud-simple/");
                });
